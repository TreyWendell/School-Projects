...
class Parser < Lexer
	...
	def statement()
		case
		when @lookahead.type == Token::PRINT
	...
	def assign()
		...
		case
		when @lookahead.type == Token::ID
            puts "Found ID Token: #{@lookahead.text}"
        end
        
        match(Token::ID)
		case
	when @lookahead.type == Token::ASSGN
	...
	def etail()
		...
		when @lookahead.type == Token::ADDOP
        ...
        when @lookahead.type == Token::SUBOP
